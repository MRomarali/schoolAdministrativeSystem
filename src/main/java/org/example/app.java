...
public class app {
    ...
    public static void populateGroupsWithStudents(List<Student> studentList) {
        ...
        for (Student currentStudent : studentList) { //For every student in the list
            ...
            subjects.addAll(currentStudent.getGradeLevel().getGradeLevelSubjects()); //create a list of the subjects (s)he is supposed to study.
            ...
            for (Subject currentSubject : subjects) { //For every subject of each student
                ...
                if (groupsBySubject.size() > 0) { //If there are any,
                    ...
                    groupsBySubject.iterator().forEachRemaining(group -> {
                        if (!(schoolClassByStudent.get(currentStudent) == null) && group.getClassesList().contains(schoolClassByStudent.get(currentStudent))) {   //keep the ones that are created for the student's class
                    ...
                    for (int i = 0; i < groupsBySubject.size(); i++) {
                        ...
                        {
                            if (groupsBySubject.get(i).getStudentsList().contains(currentStudent) || groupsBySubject.get(i).addStudentToGroup(currentStudent)) { //Add the student to the group, if the group is not full and if (s)he is not already in it (e.g. in case of a multiple-subject group)
        ...
        existingGroups.iterator().forEachRemaining(group -> {
            if (!(group.isExceedsMinimumNumber())) {
                ...
                for (Subject sub : group.getSubjectsList()) {
                    for (Student st : group.getStudentsList()) {
